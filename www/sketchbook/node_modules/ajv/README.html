...
<h2>Performance</h2><p>Ajv generates code using <a href="https://github.com/olado/doT">doT templates</a> to turn JSON schemas into super-fast validation functions that are efficient for v8 optimization.</p>
...
<h2>Install</h2><pre><code>npm install ajv
</code></pre><p>To install a stable beta version <a href="https://github.com/epoberezkin/ajv/releases/tag/5.0.3-beta.0">5.0.3</a> (see <a href="https://github.com/epoberezkin/ajv/releases/tag/5.0.1-beta.0">migration guide from 4.x.x</a>):</p>
<pre><code>npm install ajv@^5.0.3-beta
</code></pre><h2><a name="usage"></a>Getting started</h2><p>Try it in the node REPL: <a href="https://tonicdev.com/npm/ajv">https://tonicdev.com/npm/ajv</a></p>
...
<h2>Using in browser</h2><p>You can require Ajv directly from the code you browserify - in this case Ajv will be a part of your bundle.</p>
...
<h2>Command line interface</h2><p>CLI is available as a separate npm package <a href="https://github.com/jessedc/ajv-cli">ajv-cli</a>. It supports:</p>
<ul>
...
<h2>Validation keywords</h2><p>Ajv supports all validation keywords from draft 4 of JSON-schema standard:</p>
<ul>
...
<h2>Formats</h2><p>The following formats are supported for string validation with &quot;format&quot; keyword:</p>
<ul>
...
<h2>$data reference</h2><p>With <code>v5</code> option you can use values from the validated data as the values for the schema keywords. See <a href="https://github.com/json-schema/json-schema/wiki/$data-(v5-proposal">v5 proposal</a>) for more information about how it works.</p>
...
<h2>Defining custom keywords</h2><p>The advantages of using custom keywords are:</p>
<ul>
...
<h2>Asynchronous compilation</h2><p>During asynchronous compilation remote references are loaded using supplied function. See <code>compileAsync</code> method and <code>loadSchema</code> <a href="#options">option</a>.</p>
<p>Example:</p>
...
<h2>Asynchronous validation</h2><p>Example in node REPL: <a href="https://tonicdev.com/esp/ajv-asynchronous-validation">https://tonicdev.com/esp/ajv-asynchronous-validation</a></p>
...
<h3>Using transpilers with asyncronous validation functions.</h3><p>To use a transpiler you should separately install it (or load its bundle in the browser).</p>
<p>Ajv npm package includes minified browser bundles of regenerator and nodent in dist folder.</p>
<h4>Using nodent</h4><pre><code class="language-javascript">var ajv = new Ajv({ /* async: &#39;es7&#39;, */ transpile: &#39;nodent&#39; });
var validate = ajv.compile(schema); // transpiled es7 async function
validate(data).then(successFunc).catch(errorFunc);
</code></pre>
<p><code>npm install nodent</code> or use <code>nodent.min.js</code> from dist folder of npm package.</p>
<h4>Using regenerator</h4><pre><code class="language-javascript">var ajv = new Ajv({ /* async: &#39;es7&#39;, */ transpile: &#39;regenerator&#39; });
var validate = ajv.compile(schema); // transpiled es7 async function
validate(data).then(successFunc).catch(errorFunc);
</code></pre>
<p><code>npm install regenerator</code> or use <code>regenerator.min.js</code> from dist folder of npm package.</p>
<h4>Using other transpilers</h4><pre><code class="language-javascript">var ajv = new Ajv({ async: &#39;es7&#39;, transpile: transpileFunc });
var validate = ajv.compile(schema); // transpiled es7 async function
validate(data).then(successFunc).catch(errorFunc);
</code></pre>
<p>See <a href="#options">Options</a>.</p>
<h4>Comparison of async modes</h4><table>
<thead>
<tr>
<th>mode</th>
...
<h2>Filtering data</h2><p>With <a href="#options">option <code>removeAdditional</code></a> (added by <a href="https://github.com/andyscott">andyscott</a>) you can filter data during the validation.</p>
...
<h2>Assigning defaults</h2><p>With <a href="#options">option <code>useDefaults</code></a> Ajv will assign values from <code>default</code> keyword in the schemas of <code>properties</code> and <code>items</code> (when it is the array of schemas) to the missing properties and items.</p>
<p>This option modifies original data.</p>
...
<h2>Coercing data types</h2><p>When you are validating user inputs all your data properties are usually strings. The option <code>coerceTypes</code> allows you to have your data types coerced to the types specified in your schema <code>type</code> keywords, both to pass the validation and to use the correctly typed data afterwards.</p>
<p>This option modifies original data.</p>
...
<h2>API</h2><h5>new Ajv(Object options) -&gt; Object</h5><p>Create Ajv instance.</p>
<p>All the instance methods below are bound to the instance, so they can be used without the instance.</p>
<h5>.compile(Object schema) -&gt; Function&lt;Object data&gt;</h5><p>Generate validating function and cache the compiled schema for future use.</p>
...
<h5>.compileAsync(Object schema, Function callback)</h5><p>Asyncronous version of <code>compile</code> method that loads missing remote schemas using asynchronous function in <code>options.loadSchema</code>. Callback will always be called with 2 parameters: error (or null) and validating function. Error will be not null in the following cases:</p>
<ul>
...
<h5>.validate(Object schema|String key|String ref, data) -&gt; Boolean</h5><p>Validate data using passed schema (it will be compiled and cached).</p>
...
<h5>.addMetaSchema(Array&lt;Object&gt;|Object schema [, String key])</h5><p>Adds meta schema(s) that can be used to validate other schemas. That function should be used instead of <code>addSchema</code> because there may be instance options that would compile a meta schema incorrectly (at the moment it is <code>removeAdditional</code> option).</p>
...
<h5><a name="api-validateschema"></a>.validateSchema(Object schema) -&gt; Boolean</h5><p>Validates schema. This method should be used to validate schemas rather than <code>validate</code> due to the inconsistency of <code>uri</code> format in JSON-Schema standard.</p>
...
<h5>.getSchema(String key) -&gt; Function&lt;Object data&gt;</h5><p>Retrieve compiled schema previously added with <code>addSchema</code> by the key passed to <code>addSchema</code> or by its full reference (id). Returned validating function has <code>schema</code> property with the reference to the original schema.</p>
<h5>.removeSchema([Object schema|String key|String ref|RegExp pattern])</h5><p>Remove added/cached schema. Even if schema is referenced by other schemas it can be safely removed as dependent schemas have local references.</p>
...
<h5><a name="api-addformat"></a>.addFormat(String name, String|RegExp|Function|Object format)</h5><p>Add custom format to validate strings. It can also be used to replace pre-defined formats for Ajv instance.</p>
...
<h5><a name="api-addkeyword"></a>.addKeyword(String keyword, Object definition)</h5><p>Add custom validation keyword to Ajv instance.</p>
...
<h5>.getKeyword(String keyword) -&gt; Object|Boolean</h5><p>Returns custom keyword definition, <code>true</code> for pre-defined keywords and <code>false</code> if the keyword is unknown.</p>
<h5>.removeKeyword(String keyword)</h5><p>Removes custom or pre-defined keyword so you can redefine them.</p>
...
<h5>.errorsText([Array&lt;Object&gt; errors [, Object options]]) -&gt; String</h5><p>Returns the text with all errors in a String.</p>
...
<h5>Validation and reporting options</h5><ul>
...
<h5>Referenced schema options</h5><ul>
...
<h5>Options to modify validated data</h5><ul>
...
<li><em>coerceTypes</em>: change data type of data to match <code>type</code> keyword. See the example in <a href="#coercing-data-types">Coercing data types</a> and <a href="https://github.com/epoberezkin/ajv/blob/master/COERCION.md">coercion rules</a>. Option values:<ul>
...
<h5>Asynchronous validation options</h5><ul>
...
<h2>Validation errors</h2><p>In case of validation failure Ajv assigns the array of errors to <code>.errors</code> property of validation function (or to <code>.errors</code> property of Ajv instance in case <code>validate</code> or <code>validateSchema</code> methods were called). In case of <a href="#asynchronous-validation">asynchronous validation</a> the returned promise is rejected with the exception of the class <code>Ajv.ValidationError</code> that has <code>.errors</code> poperty.</p>
<h3>Error objects</h3><p>Each error is an object with the following properties:</p>
<ul>
...
<h3>Error parameters</h3><p>Properties of <code>params</code> object in errors depend on the keyword that failed validation.</p>
<ul>
...
<h2>Some packages using Ajv</h2><ul>
...
npm test
</code></pre><h2>Contributing</h2><p>All validation functions are generated using doT templates in <a href="https://github.com/epoberezkin/ajv/tree/master/lib/dot">dot</a> folder. Templates are precompiled so doT is not a run-time dependency.</p>
...
<h2>Changes history</h2><p>See <a href="https://github.com/epoberezkin/ajv/releases">https://github.com/epoberezkin/ajv/releases</a></p>
...
<h2>License</h2><p><a href="https://github.com/epoberezkin/ajv/blob/master/LICENSE">MIT</a></p></body></html>