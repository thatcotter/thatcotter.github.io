...
<h2>install</h2><pre><code>npm install safe-buffer
</code></pre><h2>usage</h2><p>The goal of this package is to provide a safe replacement for the node.js <code>Buffer</code>.</p>
...
<h2>api</h2><h3>Class Method: Buffer.from(array)</h3><!-- YAML
added: v3.0.0
-->
<ul>
...
<h3>Class Method: Buffer.from(arrayBuffer[, byteOffset[, length]])</h3><!-- YAML
added: v5.10.0
-->
<ul>
...
<h3>Class Method: Buffer.from(buffer)</h3><!-- YAML
added: v3.0.0
-->
<ul>
...
<h3>Class Method: Buffer.from(str[, encoding])</h3><!-- YAML
added: v5.10.0
-->
<ul>
...
<h3>Class Method: Buffer.alloc(size[, fill[, encoding]])</h3><!-- YAML
added: v5.10.0
-->
<ul>
...
<h3>Class Method: Buffer.allocUnsafe(size)</h3><!-- YAML
added: v5.10.0
-->
<ul>
<li><code>size</code> {Number}</li>
...
<h3>Class Method: Buffer.allocUnsafeSlow(size)</h3><!-- YAML
added: v5.10.0
-->
<ul>
<li><code>size</code> {Number}</li>
...
<p>A <code>TypeError</code> will be thrown if <code>size</code> is not a number.</p>
<h3>All the Rest</h3><p>The rest of the <code>Buffer</code> API is exactly the same as in node.js.
<a href="https://nodejs.org/api/buffer.html">See the docs</a>.</p>
<h2>Related links</h2><ul>
...
<h2>Why is <code>Buffer</code> unsafe?</h2><p>Today, the node.js <code>Buffer</code> constructor is overloaded to handle many different argument
...
<h3>Remote Memory Disclosure</h3><p>If an attacker can make your program call the <code>Buffer</code> constructor with a <code>Number</code>
...
<h3>Would this ever be a problem in real code?</h3><p>Yes. It&#39;s surprisingly common to forget to check the type of your variables in a
...
<h3>Which real-world packages were vulnerable?</h3><h4><a href="https://www.npmjs.com/package/bittorrent-dht"><code>bittorrent-dht</code></a></h4><p><a href="https://github.com/mafintosh">Mathias Buus</a> and I
...
<h4><a href="https://www.npmjs.com/package/ws"><code>ws</code></a></h4><p>That got us wondering if there were other vulnerable packages. Sure enough, within a short
...
<h3>What&#39;s the solution?</h3><p>It&#39;s important that node.js offers a fast way to get memory otherwise performance-critical
...
<h4>A new API: <code>Buffer.allocUnsafe(number)</code></h4><p>The functionality of creating buffers with uninitialized memory should be part of another
...
<h3>How do we fix node.js core?</h3><p>We sent <a href="https://github.com/nodejs/node/pull/4514">a PR to node.js core</a> (merged as
...
<h3>What&#39;s the real long-term fix?</h3><p>We could deprecate and remove <code>new Buffer(number)</code> and use <code>Buffer.allocUnsafe(number)</code> when
...
<h4>Update</h4><p>We now support adding three new APIs:</p>
<ul>
...
<h3>Conclusion</h3><p>We think there&#39;s a serious design issue with the <code>Buffer</code> API as it exists today. It
...
<h2>credit</h2><p>The original issues in <code>bittorrent-dht</code>
...
auditing the code.</p>
<h2>license</h2><p>MIT. Copyright (C) <a href="http://feross.org">Feross Aboukhadijeh</a></p></body></html>